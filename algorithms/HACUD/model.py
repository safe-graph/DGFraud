...
class HACUD(object):
    def __init__(self, data_config, pretrain_data):
        ...
        *********************************************************
        ...
        """
        self.n_embeddings = self._create_embed()

        """
        *********************************************************
        Establish the final representations for user-item pairs in batch.
        """
        self.n_g_embeddings = tf.nn.embedding_lookup(self.n_embeddings, self.nodes)
    ...
    def _init_weights(self):
        ...
        for n in range(self.n_metapath):
            ...
            all_weights['W_f_%d' % n] = tf.Variable(
                initializer([self.emb_dim, self.emb_dim]), name='W_f_%d' % n)
    ...
    def _create_embedding(self):
        ...
        h = tf.matmul(embeddings, self.weights['W_rho_%d' %n]) + self.weights['b_rho_%d' %n]
        ...
        for n in range(self.n_metapath):
            ...
            f_u['%d' %n] = tf.nn.leaky_relu(tf.matmul(tf.concat([h,h_u['%d' %n]],1), self.weights['W_f_%d' %n]) 
...
class NGCF(object):
    def __init__(self, data_config, pretrain_data):
        ...
        if args.alg_type == 'my':
            self.n_layers = len(self.weight_size )+1
        else:
        ...
        *********************************************************
        ...
        elif self.alg_type in ['my']:
            self.ua_embeddings, self.ia_embeddings = self._create_my_embed()
    ...
    def _create_ngcf_embed(self):
        ...
        print(all_embeddings.shape.as_list())
    ...
    def _create_gcn_embed(self):
        ...
        print(u_g_embeddings.shape.as_list())
        return u_g_embeddings, i_g_embeddings
    ...
    def _dropout_sparse(self, X, rate, n_nonzero_elems):
        """
...
if __name__ == '__main__':
    ...
    *********************************************************
    ...
    for epoch in range(args.epoch):
        ...
        test_, test_batch_loss, test_batch_mf_loss, test_batch_emb_loss, test_batch_reg_loss = sess.run([model.opt, model.loss, model.mf_loss, model.emb_loss, model.reg_loss],
        ...
        if np.isnan(loss) == True:
            ...
            print('mf_loss =%s' % mf_loss)
            sys.exit()
        
        # print the test evaluation metrics each 10 epochs; pos:neg = 1:10.
        if (epoch + 1) % 10 != 0:
            if args.verbose > 0 and epoch % args.verbose == 0:
                perf_str = 'Epoch %d [%.1fs]: train loss==[%.5f=%.5f + %.5f]' % (
        ...
        cur_best_pre_0, stopping_step, should_stop = early_stopping(ret['recall'][0], cur_best_pre_0,
                                                                    stopping_step, expected_order='acc', flag_step = early_stop)
