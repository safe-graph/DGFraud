...
class Data(object):
    def __init__(self, path, save_path):
        ...
        for i in range(self.n_metapath):
            ...
            u_index.append(z[0])
            v_index.append(z[1])   
            self.n_int.append(z[0])
        
        self.print_statistics()

        self.R = sp.dok_matrix((self.n_nodes, self.n_nodes), dtype=np.float32)
        
        for i in range(len(u_index)):
            self.R[u_index, v_index] = 1
 
    def get_adj_mat(self):
        try:
            t1 = time()
            adj_mat = sp.load_npz(self.save_path + '/'+ '/s_adj_mat.npz')
            norm_adj_mat = sp.load_npz(self.save_path + '/s_norm_adj_mat.npz')
            mean_adj_mat = sp.load_npz(self.save_path + '/s_mean_adj_mat.npz')
            print('already load adj matrix', adj_mat.shape, time() - t1)

        except Exception:
            adj_mat, norm_adj_mat, mean_adj_mat = self.create_adj_mat()
            sp.save_npz(self.save_path + '/s_adj_mat.npz', adj_mat)
            sp.save_npz(self.save_path + '/s_norm_adj_mat.npz', norm_adj_mat)
            sp.save_npz(self.save_path + '/s_mean_adj_mat.npz', mean_adj_mat)
    ...
    def print_statistics(self):
        ...
        print('n_train=%d, n_test=%d, sparsity=%.5f' % (self.n_train, self.n_test, (np.array(self.n_int)/(self.n_nodes * self.n_nodes))))
